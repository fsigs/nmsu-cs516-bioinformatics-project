...
class FrequentKmers:
  ...
  # Removes polyA tail
  def remove_polyA(self, sequence, tail_length=10, tail_char='A'):
    pattern = tail_char + "{" + str(tail_length) + ",}$"
  ...
  def get_covid_genome(self, filename, tail_length=None, tail_char='A'):
    with open(filename, 'r') as file:
      sequence = file.read().replace('\n', '').strip()
    if tail_length is not None:
      return self.remove_polyA(sequence, tail_length, tail_char)
    else:
      return self.remove_polyA(sequence)
